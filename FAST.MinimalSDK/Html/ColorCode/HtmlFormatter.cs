...
using FAST.Html.ColorCode.Common;
using FAST.Html.ColorCode.Parsing;
using FAST.Html.ColorCode.Styling;
using System.Net;
using System.Text;

namespace FAST.Html.ColorCode
{
    /// <summary>
    /// Creates a <see cref="HtmlFormatter"/>, for creating HTML to display Syntax Highlighted code.
    /// </summary>
    internal /*public*/ class HtmlFormatter : CodeColorizerBase
    {
        /// <summary>
        /// Creates a <see cref="HtmlFormatter"/>, for creating HTML to display Syntax Highlighted code.
        /// </summary>
        ...
        private void WriteElementStart(string elementName, string foreground = null, string background = null, bool italic = false, bool bold = false)
        {
            ...
            if (!string.IsNullOrWhiteSpace(foreground) || !string.IsNullOrWhiteSpace(background) || italic || bold)
            {
                ...
                if (!string.IsNullOrWhiteSpace(foreground))
                    Writer.Write("color:{0};", htmlColorHelper.toHtmlColor(foreground));

                if (!string.IsNullOrWhiteSpace(background))
                    Writer.Write("background-color:{0};", htmlColorHelper.toHtmlColor(background));
