...
namespace FAST.Types
{
    ...
    public static class rfc2822DateTime
	{
		...
		private static DateTime adjustTimezone(DateTime dateTime, string dateInput)
		{
			...
			fastLogger.log.debug("No timezone found in date: " + dateInput + ". Using -0000 as default.");
		...
		private static DateTime extractDateTime(string dateInput)
		{
			...
			if (CustomDateTimeFormats != null && CustomDateTimeFormats.Length > 0)
			{
				...
				try
				{
					...
					fastLogger.log.debug(String.Format("Successfully parsed date input \"{0}\" using a custom format. Converted to date: {1}", dateInput, dateTime.ToString()));
					return dateTime;
				}
			...
			if (match.Success)
			{
				...
				{
					throw new Exception("The given date appeared to be in a valid format, but could not be converted to a DateTime object: " + dateInput);
				}
			}
			else
			{
				fastLogger.log.error("The given date does not appear to be in a valid format: " + dateInput);
			}
		...
		private static void validateDayNameIfAny(DateTime dateTime, string dateInput)
		{
			...
			if (dateInput.Length >= 4 && dateInput[3] == ',')
			{
				...
				{
					fastLogger.log.debug("Day-name does not correspond to the weekday of the date: " + dateInput);					
				}
			}
